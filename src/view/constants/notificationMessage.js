...
const NOTIFICATION_MESSAGE = deepFreeze({
  opening: `안녕하세요 ! 우테코 식당 ${DECEMBER_EVENT.month}월 이벤트 플래너입니다.\n`,
  resultHeader: (day) =>
    `${DECEMBER_EVENT.month}월 ${day}일에 우테코 식당에서 받을 이벤트 혜택 미리 보기!\n`,
  orderId: (orderId) => `고객님의 주문 번호는 ${orderId}입니다.\n`,
});

const WARNING_MESSAGE = deepFreeze({
  header: '[ 주의 사항 ]',
  minOrderPrice: `⚠️ 총 주문 금액 ${addMoneyDelimiter(MIN_ORDER_PRICE)}${
    PREVIEW_MARK.price
  } 이상부터 이벤트가 적용됩니다.`,
  beverage: '⚠️ 음료만 주문 시, 주문할 수 없습니다.',
  menuCount: `⚠️ 메뉴는 한 번에 최대 ${MENU_COUNT.max}개까지만 주문할 수 있습니다.`,
